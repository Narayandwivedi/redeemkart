...
const UserSelling = () => {
  ...
  const handleRemoveUserListing = async (id) => {
    if (!window.confirm('Remove this card?')) return
    try {
      await axios.patch(`${BACKEND_URL}/api/admin/gift-cards/${id}/remove`, {}, { withCredentials: true })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 border border-gray-100">
            ...
            <div className="flex items-center justify-between gap-2 mt-6">
              <button
                onClick={() => handleRemoveUserListing(approveModalCard._id)}
                disabled={approveLoading}
                className="px-4 py-2.5 text-sm font-semibold border border-red-200 rounded-xl text-red-600 hover:bg-red-50 disabled:opacity-50"
              >
                Remove Card
              </button>
